package com.example.appdemowithjson;

import android.os.AsyncTask;

import org.json.JSONArray;
import org.json.JSONException;
import org.json.JSONObject;

import java.io.BufferedReader;
import java.io.IOException;
import java.io.InputStream;
import java.io.InputStreamReader;
import java.net.HttpURLConnection;
import java.net.MalformedURLException;
import java.net.URL;

public class FetchData extends AsyncTask<Void, Void, Void> {

    String data = "";
    String urlData = "https://github.com/Oyedee/JsonObjectDemo/user.json";
    String singleParsed = "", dataParsed = "";
    @Override
    protected Void doInBackground(Void... voids) {
        try {
            //to fetch json data from the url provided else catch
            URL url = new URL(urlData);
            HttpURLConnection httpURLConnection = (HttpURLConnection) url.openConnection();
            InputStream inputStream = httpURLConnection.getInputStream();
            //Read the data from the input Stream using a bufferedReader
            BufferedReader bufferedReader = new BufferedReader(new InputStreamReader(inputStream));

            //fetch the data from the bufferedReader into an empty String
            String line = "";

            while (line != null) {
                line = bufferedReader.readLine();
                data = data + line;
            }
            //let's create a jsonArray to sought our fetched data. This will make is a more organized data
            JSONArray jsonArray = new JSONArray(data);
            for(int index = 0; index < jsonArray.length(); index++) {
                JSONObject jsonObject = (JSONObject) jsonArray.get(index);

                singleParsed = "name: " + jsonObject.get("name") + "\n"
                        + "address: " + jsonObject.get("address") + "\n"
                        + "isProgrammer: " + jsonObject.get("isProgrammer");
                //combine each single parsed object together with the total object in the array
                dataParsed = dataParsed + singleParsed;
            }
        } catch (MalformedURLException e) {
            e.printStackTrace();
        } catch (IOException e) {
            e.printStackTrace();
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return null;
    }

    @Override
    protected void onPostExecute(Void unused) {
        MainActivity.displayData.setText(dataParsed);
        super.onPostExecute(unused);
    }
}
